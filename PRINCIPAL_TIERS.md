...
| ListPlans | **SP** | `GET /api/v1/subscription/plans`. Was UP on the reasoning below — "public catalog data, but now genuinely reachable only by an authenticated user." That stopped being true on 2026-09-25: the marketing landing page prints live pricing to logged-out visitors, so the Gateway route is permitAll and the call arrives with the internal secret and no user. Structurally identical to org-service's `SignUp` — the secret is what keeps the gRPC call internal, not a JWT. Authenticated callers are unaffected (UP satisfies an SP floor). **Caveat:** this RPC's own `include_private` filter trusts `ServicePrincipal`, and a relayed member of the public now presents as one; it is safe only because api-gateway pins `setIncludePrivate(false)` and is the RPC's sole caller. Re-derive that check before anything starts setting the flag. |
...
Revised from the scaffold-time design: subscription-service's own `PreviewPlanChange` — already
built, already calling this service via `GrpcBillingClient` — reads `ListInvoices` (the proration
credit's amount-paid lookup) and `GetBillingAccount` (the VAT-payer estimate) as a SERVICE-tier
internal caller, with no Gateway-forwarded caller for either RPC yet. Marking them SP doesn't weaken
anything a real future customer read would need — the tier check is a minimum
(`actual.ordinal() >= required.ordinal()`), so a genuine USER-tier caller still passes an SP-required
check trivially; leaving them at UP would have made `GrpcBillingClient` silently fall back to its
"no invoice" / "VAT payer" defaults on every single call once enforcement is on — exactly the quiet,
hard-to-notice failure this checklist exists to catch. Caught and fixed while wiring the gRPC layer
(Phase 2c §2 step 2), not after the first `PERMISSION_DENIED`.
